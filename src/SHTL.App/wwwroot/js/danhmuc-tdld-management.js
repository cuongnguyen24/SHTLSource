/**
 * DanhMucTDLD Management JavaScript
 * M0076 — Handles CRUD operations for CategoryTypes and Categories (scope=Recruitment)
 * Cloned from categorytype-management.js with URLs pointing to /DanhMucTDLD/
 */
...
function initializeEventHandlers() {
    ...
    $('#btnConfirmDelete').click(function () {
        ...
        const url = isBulkDelete ? '/DanhMucTDLD/DeleteMultiple' : '/DanhMucTDLD/Delete/' + deleteId;
        const method = isBulkDelete ? 'DELETE' : 'DELETE'; // Reverted to original
...
function loadCategories(categoryTypeId) {
    ...
    $.ajax({
        url: `/DanhMucTDLD/GetCategories/${categoryTypeId}`,
        type: 'GET',
        success: function (response) {
            $('#categoriesTableBody').empty();
...
function saveCategoryType() {
    ...
    const url = modalMode === 'edit'
        ? '/DanhMucTDLD/UpdateWithCategories/' + editingCategoryTypeId
        : '/DanhMucTDLD/Create';
    const method = modalMode === 'edit' ? 'PUT' : 'POST'; // Reverted
