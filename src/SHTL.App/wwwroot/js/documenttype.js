...
$(document).ready(function () {
    ...
    if (currentPath.includes('/documenttype') && !currentPath.includes('/create') && !currentPath.includes('/edit')) {
        initializeIndex();
    }
...
function initializeIndex() {
    ...
    const _ocrFunctionNames = window.ocrFunctionNames || {};
    ...
    /**
     * Initialize DataTable for document types
     */
    function initializeDataTable() {
        table = $('#documentTypesTable').dataTableFigma({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        const name = _ocrFunctionNames[data] || 'Unknown';
                        return `<span class="badge-figma badge-figma-info">${escapeHtml(name)}</span>`;
                    }
                },
                {
                    data: 'metadataFields',
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        // View action
                        html += `<a href="/DocumentType/Edit/${row.id}?readonly_mode=true" class="btn-action-figma btn-action-view" title="Xem chi tiết"><i class="fas fa-eye"></i></a>`;

                        if (window.canUpdate) {
                            html += `<a href="/DocumentType/Edit/${row.id}" class="btn-action-figma btn-action-edit" title="Chỉnh sửa"><i class="fas fa-pen"></i></a>`;
                        }
    ...
    function initializeEventHandlers() {
        ...
        // Delete button
        $('#documentTypesTable').on('click', '.btn-delete', function () {
        ...
        $('#btnConfirmDelete').click(function () {
            ...
            $.ajax({
                url: `/DocumentType/Delete/${deleteId}`,
                type: 'DELETE',
                success: function (response) {
                    toastr.success(`Đã xóa loại tài liệu "${deleteName}" thành công!`);
                    $('#deleteModal').modal('hide');
                    table.ajax.reload();
                },
                error: function (xhr) {
                    const message = xhr.responseJSON?.message || 'Không thể xóa loại tài liệu';
...
function initializeForm() {
    // Initialize data from window
    _ocrFunctionNames = window.ocrFunctionNames || {};
    _ocrFieldMappings = window.ocrFieldMappings || {};
    ...
    $('#code').on('blur', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                const exists = response.data && response.data.some(dt => dt.code.toLowerCase() === code.toLowerCase());
    ...
    // OCR function change - update OCR mapping field options
    $('#ocrFunction').on('change', function () {
        const ocrFunctionValue = parseInt($(this).val());
        updateOCRMappingOptions(ocrFunctionValue);
    });
    ...
    $('#documentTypeForm').on('submit', function (e) {
        ...
        try {
            const documentTypeId = $('#documentTypeId').val();
            if (documentTypeId) {
    ...
    function addMetadataFieldRow(field = null) {
        ...
        // Get current OCR function value for mapping options
        const ocrFunctionValue = parseInt($('#ocrFunction').val());
        const ocrMappingOptions = getOCRMappingOptionsHTML(ocrFunctionValue, field?.ocrMappingField);
        ...
        const row = `<tr id="${rowId}" data-field-id="${fieldId || ''}" class="align-middle">
            ...
            <td>
                <select class="input-figma input-figma-sm field-ocrmapping select2-in-table" ${isReadOnlyMode ? 'disabled' : ''}>
                    ${ocrMappingOptions}
                </select>
            </td>
    ...
    function getOCRMappingOptionsHTML(ocrFunction, selectedValue = null) {
        let html = '<option value="">-- Không mapping --</option>';
        const availableFields = _ocrFieldMappings[ocrFunction] || [];

        availableFields.forEach(function (fieldCode) {
            // Convert field code to readable label (e.g., full_name -> Full Name)
            const label = fieldCode.split('_').map(word =>
                word.charAt(0).toUpperCase() + word.slice(1)
            ).join(' ');
            const selected = selectedValue === fieldCode ? 'selected' : '';
            html += `<option value="${fieldCode}" ${selected}>${label}</option>`;
        });

        return html;
    }

    function updateOCRMappingOptions(ocrFunction) {
        console.log('Updating OCR mapping options for:', ocrFunction);

        // Update all OCR mapping dropdowns in metadata fields
        $('#metadataFieldsTable tbody tr').each(function () {
            const $select = $(this).find('.field-ocrmapping');
            if ($select.length === 0) return;

            const currentValue = $select.val();
            const newOptionsHTML = getOCRMappingOptionsHTML(ocrFunction, currentValue);
    ...
    function collectFormData() {
        ...
        $('#metadataFieldsBody tr').each(function () {
            ...
            const field = {
                ...
                OCRMappingField: row.find('.field-ocrmapping').val() || null,
                DefaultValue: null,
        ...
        return {
            ...
            OCRFunction: parseInt($('#ocrFunction').val()),
    ...
    function createDocumentType() {
        ...
        $.ajax({
            ...
            success: function (response) {
                toastr.success('Tạo mới loại tài liệu thành công', 'Thành công');
                setTimeout(function () {
                    window.location.href = '/DocumentType';
                }, 1000);
            },
            error: function (xhr) {
                console.error('Error creating document type:', xhr);
                const message = xhr.responseJSON?.message || 'Không thể tạo mới loại tài liệu';
                toastr.error(message, 'Lỗi');
            },
            complete: function () {
                isSubmitting = false;
    ...
    function updateDocumentType(id) {
        ...
        $.ajax({
            url: `/DocumentType/Update/${id}`,
            type: 'PUT',
            contentType: 'application/json',
            data: JSON.stringify(data),
            success: function (response) {
                toastr.success('Cập nhật loại tài liệu thành công', 'Thành công');
                setTimeout(function () {
                    window.location.href = '/DocumentType';
                }, 1000);
            },
            error: function (xhr) {
                console.error('Error updating document type:', xhr);
                const message = xhr.responseJSON?.message || 'Không thể cập nhật loại tài liệu';
                toastr.error(message, 'Lỗi');
            },
            complete: function () {
                isSubmitting = false;
    ...
    // Load existing document type data if in edit mode (at the end after events are bound)
    if (window.existingDocumentType) {
        const docType = window.existingDocumentType;
        if (docType.metadataFields && docType.metadataFields.length > 0) {
            docType.metadataFields.sort((a, b) => a.displayOrder - b.displayOrder)
