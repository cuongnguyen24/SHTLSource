/**
 * VSIC2018 Management JavaScript
 * Pattern: server-side search/pagination via quickSearch form → _VSIC2018 partial
 */
(function () {
    'use strict';
    ...
    $(document).on('click', '#btnAddItem', function () {
        resetForm();
        $('#modalTitle').html('<i class="fas fa-industry mr-2"></i>Thêm mới VSIC2018');
        $('#code').prop('readonly', false);
        $('#vsic2018Modal').modal('show');
    });
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.ajax({
            url: '/VSIC2018/Details/' + id,
            type: 'GET',
            success: function (data) {
                $('.is-invalid').removeClass('is-invalid');
                $('.invalid-feedback').remove();

                $('#vsic2018Id').val(data.id);
                $('#code').val(data.code).prop('readonly', true);
                $('#name').val(data.name);
                $('#description').val(data.description || '');
                $('#displayOrder').val(data.displayOrder);

                $('#modalTitle').html('<i class="fas fa-industry mr-2"></i>Chỉnh sửa VSIC2018');
                $('#vsic2018Modal').modal('show');
            },
            error: function (xhr) {
    ...
    $(document).on('click', '#btnSaveVSIC2018', function () {
        if (isSubmitting) return;
        if (!validateForm()) return;

        const id = $('#vsic2018Id').val();
        const data = {
            code: $('#code').val().trim(),
            name: $('#name').val().trim(),
        ...
        isSubmitting = true;
        $('#btnSaveVSIC2018').prop('disabled', true)
        ...
        $.ajax({
            ...
            complete: function () {
                isSubmitting = false;
                $('#btnSaveVSIC2018').prop('disabled', false)
    ...
    $(document).on('click', '#btnConfirmDelete', function () {
        ...
        const url = isBulkDelete
            ? '/VSIC2018/DeleteMultiple'
            : '/VSIC2018/Delete/' + deleteId;
        const method = 'DELETE';
    ...
    function validateForm() {
        ...
        if (!code) { showError('#code', 'Mã ngành là bắt buộc'); isValid = false; }

        const name = $('#name').val().trim();
        if (!name) { showError('#name', 'Tên ngành là bắt buộc'); isValid = false; }
