/**
 * Business Type Management JavaScript
 * Pattern: server-side search/pagination via quickSearch form → _BusinessTypes partial
 */
(function () {
    'use strict';
    ...
    function reloadList() {
        $('#frmBusinessType').trigger('submit');
    }
    ...
    $(document).on('click', '#btnAddBusinessType', function () {
        resetForm();
        $('#modalTitle').html('<i class="fas fa-building mr-2"></i>Thêm mới Loại hình Doanh nghiệp');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.ajax({
            url: '/BusinessTypes/Details/' + id,
            type: 'GET',
            success: function (data) {
                $('.is-invalid').removeClass('is-invalid');
                $('.invalid-feedback').remove();

                $('#businessTypeId').val(data.id);
                $('#code').val(data.code).prop('readonly', true);
                $('#name').val(data.name);
                $('#description').val(data.description || '');
                $('#displayOrder').val(data.displayOrder);

                $('#modalTitle').html('<i class="fas fa-building mr-2"></i>Chỉnh sửa Loại hình Doanh nghiệp');
                $('#businessTypeModal').modal('show');
            },
            error: function (xhr) {
                console.error('Error loading business type details:', xhr);
                toastr.error('Không thể tải thông tin loại hình doanh nghiệp', 'Lỗi');
            }
        });
    });
    ...
    $(document).on('click', '#btnSaveBusinessType', function () {
        ...
        const url = id
            ? '/BusinessTypes/Update/' + id
            : '/BusinessTypes/Create';

        isSubmitting = true;
        $('#btnSaveBusinessType').prop('disabled', true)
        ...
        $.ajax({
            ...
            error: function (xhr) {
                console.error('Error saving business type:', xhr);
                if (xhr.responseJSON) {
                ...
                toastr.error('Có lỗi xảy ra khi lưu loại hình doanh nghiệp', 'Lỗi');
            },
            complete: function () {
                isSubmitting = false;
                $('#btnSaveBusinessType').prop('disabled', false)
    ...
    $(document).on('click', '#btnConfirmDelete', function () {
        ...
        const url = isBulkDelete
            ? '/BusinessTypes/DeleteMultiple'
            : '/BusinessTypes/Delete/' + deleteId;
        const method = 'DELETE';
        ...
        $.ajax({
            ...
            error: function (xhr) {
                console.error('Error deleting business type:', xhr);
    ...
    function validateForm() {
        ...
        if (!code) { showError('#code', 'Mã loại hình là bắt buộc'); isValid = false; }

        const name = $('#name').val().trim();
        if (!name) { showError('#name', 'Tên loại hình là bắt buộc'); isValid = false; }
