...
using SHTL.Modules.Features.Export.Exporters;
using SHTL.Modules.Shared.Contracts;

namespace SHTL.Modules.Features.Export;
...
public class ExportWorker : BackgroundService
{
    ...
    private async Task ProcessJobAsync(SHTL.Modules.Core.Domain.Entities.Stg.ExportJob job)
    {
        try
        {
    ...
    private BaseExporter? CreateExporter(SHTL.Modules.Core.Domain.Entities.Stg.ExportJob job, SHTL.Modules.Core.Domain.Entities.Stg.ExportType exportType)
    {
    ...
    private static string BuildCsv(IEnumerable<SHTL.Modules.Core.Domain.Entities.Stg.Document> docs)
    {
        ...
        sb.AppendLine("Id,Name,SymbolNo,RecordNo,IssuedBy,Author,CurrentStep,Status,Created,CreatedBy,FilePath");
