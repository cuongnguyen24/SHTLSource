...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Login(LoginRequest model, string? returnUrl)
    {
        ...
        {
            _logger.LogError(ex, "Lỗi khi đăng nhập (thường là SQL Server hoặc thiếu ConnectionStrings:DefaultConnection trong appsettings).");
            AppDataErrorLogger.WriteException(_env, ex, "Account/Login");
            var msg = _env.IsDevelopment()
                ? ex.Message
                : "Không thể kết nối cơ sở dữ liệu. Xem chi tiết lỗi SQL trong thư mục app-data/logs (errors-YYYYMMDD.log) cạnh ứng dụng, và kiểm tra appsettings.json (ConnectionStrings:DefaultConnection).";
