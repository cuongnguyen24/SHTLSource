...
internal static class PdfOcrRegionTextExtractor
{
    ...
    private static IEnumerable<Rectangle> BuildRegionCandidates(
        PdfPage page, decimal xRatio, decimal yRatio, decimal widthRatio, decimal heightRatio)
    {
        ...
        var c1 = BuildRegion(crop, xRatio, yRatio, widthRatio, heightRatio, true);
        var c2 = BuildRegion(crop, xRatio, yRatio, widthRatio, heightRatio, false);
        var c3 = BuildRegion(media, xRatio, yRatio, widthRatio, heightRatio, true);

        if (c1 is not null) yield return c1;
        if (c2 is not null) yield return c2;
        if (c3 is not null) yield return c3;
    }

    private static Rectangle? BuildRegion(
        Rectangle box, decimal xRatio, decimal yRatio, decimal widthRatio, decimal heightRatio, bool yFromTop)
    {
