...
internal sealed class Pdf2WorkerHostedService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation(
            "Pdf2Layer khởi động. File log (ưu tiên): {Paths}",
            string.Join(" | ", logRoots));
        await AppDataFileLog.WriteAsync(
            "INFO",
            $"Pdf2Layer service started. Ghi log theo ngày tại: {string.Join(" | ", logRoots)} (thư mục yyyy/MM, file pdf2layer-yyyyMMdd.log). " +
            $"Nếu không ghi được, fallback: {Path.Combine(Path.GetTempPath(), "SHTL", "Pdf2Layer", "AppData")}.")
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (activeCount == 0 && DateTime.UtcNow - lastStaleReset > TimeSpan.FromMinutes(5))
                {
                    ...
                    var reset = await _repo.ResetStaleSearchablePdfProcessingAsync(staleWindow, stoppingToken).ConfigureAwait(false);
                    if (reset > 0)
                    {
                        _logger.LogWarning("Pdf2Layer: phục hồi {Count} job bị kẹt (stale processing)", reset);
                ...
                if (DateTime.UtcNow - lastResourceLog > TimeSpan.FromMinutes(1))
                {
                    ...
                    _logger.LogInformation(
                        "Pdf2Layer: workers={Active}/{Target} | freeMemory={FreeMem}MB | cpuCores={Cores}",
            ...
            {
                _logger.LogError(ex, "Pdf2Layer worker loop exception");
        ...
        {
            _logger.LogInformation("Pdf2Layer: đợi {Count} job đang chạy hoàn tất trước khi tắt...", stillRunning.Count);
        ...
        await AppDataFileLog.WriteAsync("INFO", "Pdf2Layer service stopped.").ConfigureAwait(false);
    }
    ...
    private async Task RunJobSafeAsync(long jobId, CancellationToken stoppingToken)
    {
        ...
        catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
        {
            ...
            _logger.LogInformation("Pdf2Layer: job #{JobId} bị hủy do service dừng", jobId);
            await AppDataFileLog.WriteAsync(
                "INFO",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Pdf2Layer: lỗi không mong đợi trong job #{JobId}", jobId);
