...
internal sealed class PythonDependencyBootstrapper
{
    ...
    public async Task<bool> EnsureReadyAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError("Pdf2Layer: thiếu package Python (vncv/fitz) và AutoInstallPythonDependencies=false.");
        ...
        _logger.LogWarning("Pdf2Layer: thiếu dependency Python, bắt đầu tự cài...");
        var pipReady = await EnsurePipAsync(python, cancellationToken).ConfigureAwait(false);
        if (!pipReady)
        {
            _logger.LogError("Pdf2Layer: không thể khởi tạo pip.");
        ...
        {
            _logger.LogError("Pdf2Layer: không tìm thấy requirements file {Path}", requirements);
        ...
        {
            _logger.LogError("Pdf2Layer: tự cài dependency thất bại.");
        ...
        {
            _logger.LogError("Pdf2Layer: verify dependency thất bại.");
        ...
        _logger.LogInformation("Pdf2Layer: dependency Python đã sẵn sàng.");
        return true;
    }
    ...
    private async Task<string?> EnsurePythonRuntimeAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError("Pdf2Layer: máy đích chưa có Python runtime và AutoProvisionEmbeddedPython=false.");
    ...
    private async Task<bool> InstallEmbeddedPythonAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            _logger.LogInformation("Pdf2Layer: tải Python embeddable từ {Url}", downloadUrl);
            ...
            _logger.LogInformation("Pdf2Layer: đã giải nén Python embeddable vào {Path}", pythonDir);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Pdf2Layer: không thể tải/cài Python embeddable");
    ...
    private async Task<bool> EnsurePipAsync(string pythonExecutable, CancellationToken cancellationToken)
    {
        ...
        try
        {
            _logger.LogInformation("Pdf2Layer: tải get-pip.py từ {Url}", _options.Value.GetPipUrl);
        ...
        {
            _logger.LogError(ex, "Pdf2Layer: cài pip thất bại");
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Pdf2Layer: không khởi chạy Python executable {Path}", executable);
        ...
        {
            _logger.LogWarning(
                "Pdf2Layer: lệnh Python thất bại (code={Code}). out={Out}; err={Err}",
                proc.ExitCode,
                stdout.Trim(),
                stderr.Trim());
